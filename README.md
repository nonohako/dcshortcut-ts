...
- 모든 탭의 URL·제목을 읽을 수 있는 `tabs` 권한을 제거했습니다.
- 자동 새로고침 리더를 DC 콘텐츠 스크립트의 상태 응답으로 선정하도록 변경했습니다.
- DC 탭 전파, 페이지 이동 감지, 매크로와 일반 사이트 즐겨찾기 UI가 `activeTab` 구조에서도 유지되도록 보강했습니다.
- 빠르게 탭을 전환할 때 늦게 도착한 응답이 현재 리더를 덮는 경쟁 조건을 방지했습니다.
- 일반 사이트의 전역 CSS가 즐겨찾기·설정 UI에 침범하지 않도록 Shadow DOM으로 격리했습니다.
...
- 광범위한 호스트 권한을 제거하고, 팝업 버튼이나 Chrome 단축키를 직접 사용한 현재 탭에만 UI를 주입합니다.
- `Alt + Z`·`Alt + X` 매크로 단축키도 다른 기능처럼 앱 설정에서 조합과 ON/OFF를 관리합니다.
- 설정의 모든 스위치를 한 번에 켜거나 끌 수 있습니다.
- 일반 웹사이트도 현재 페이지의 제목과 주소로 즐겨찾기에 추가할 수 있습니다.
- 즐겨찾기 삭제·이동·순서 변경을 7초 안에 되돌릴 수 있습니다.
- 설정창에서 Chrome에 실제 지정된 즐겨찾기 단축키와 미지정 상태를 표시합니다.
