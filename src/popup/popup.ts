...
const shortcutData = {
    ...
    customizable: {
        W: '글쓰기', C: '댓글 입력', D: '댓글 이동', R: '새로고침',
        Q: '최상단 스크롤', E: '글 목록 스크롤', F: '전체글', G: '개념글',
        A: '다음 페이지', S: '이전 페이지', Z: '다음 글', X: '이전 글',
        PrevProfile: '이전 프로필', NextProfile: '다음 프로필',
...
const customShortcutListEl = document.getElementById('custom-shortcut-list') as HTMLUListElement | null;
const fixedShortcutListEl = document.getElementById('fixed-shortcut-list') as HTMLUListElement | null;
...
async function loadSettings(): Promise<Record<string, string>> {
    ...
    const keysToGet = Object.fromEntries(
        Object.keys(defaultKeys).map(action => [`shortcut${action}Key`, defaultKeys[action as ShortcutAction]])
    );
