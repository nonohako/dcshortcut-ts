...
export function addPrefetchHints(): void {
    ...
    document.querySelectorAll<HTMLLinkElement>('link[data-dc-prefetch="true"]').forEach(link => link.remove());
    ...
    const exceptionPagingWrap: Element | null = document.querySelector('.bottom_paging_wrap.re, .bottom_paging_wrapre');
    ...
    if (targetPagingBox) {
        ...
        if (currentPageElement) {
            ...
            prevPageLinkHref = targetPagingBox.querySelector<HTMLAnchorElement>('a.search_prev[href]')?.href ?? null;
            nextPageLinkHref = targetPagingBox.querySelector<HTMLAnchorElement>('a.search_next[href]')?.href ?? null;
        }
    ...
    if (currentPostIcon) {
        ...
        while (prevRow) {
            ...
            if (Posts.isValidPost(prevRow.querySelector('td.gall_num'), prevRow.querySelector('td.gall_tit'), prevRow.querySelector('td.gall_subject'))) {
                prevPostLinkHref = prevRow.querySelector<HTMLAnchorElement>('td.gall_tit a:first-child')?.href ?? null;
                break; // 찾았으면 반복 중단
            }
        ...
        while (nextRow) {
            ...
            if (Posts.isValidPost(nextRow.querySelector('td.gall_num'), nextRow.querySelector('td.gall_tit'), nextRow.querySelector('td.gall_subject'))) {
                nextPostLinkHref = nextRow.querySelector<HTMLAnchorElement>('td.gall_tit a:first-child')?.href ?? null;
                break; // 찾았으면 반복 중단
            }
