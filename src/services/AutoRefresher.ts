...
interface AutoRefresherType {
  timerId: number | null;
  settingsStore: SettingsStore | null;
  postsModule: PostsModule | null;
...
const AutoRefresher: AutoRefresherType = {
  timerId: null,
  settingsStore: null,
  postsModule: null,
  eventsModule: null,
  ...
  async checkNewPosts(currentGenerationId: number): Promise<void> {
    ...
    try {
      ...
      currentTbody.querySelectorAll('tr').forEach((row) => {
        if (
          this.postsModule!.isValidPost(
            row.querySelector('td.gall_num'),
  ...
  insertAndTrimPosts(
    ...
    if (existingColumnCount === 0) return;
    ...
    newRowsFromFetch.forEach((newRowSource) => {
      ...
      if (newColumnCount < existingColumnCount) {
        ...
        console.log(
          `[AutoRefresher] 구조 불일치 감지. ${columnsToAdd}개의 체크박스 열을 생성하여 보정합니다.`
        );
        for (let i = 0; i < columnsToAdd; i++) {
          ...
          newRow.insertBefore(checkboxTd, newRow.firstChild);
        }
      }
