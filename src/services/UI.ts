 ...
 */
type ElementProperties<K extends keyof HTMLElementTagNameMap> = Partial<Omit<HTMLElementTagNameMap[K], 'style' | 'dataset'>> & {
...
const UI = {
    ...
    removeAlert(message?: string): void {
        ...
        const selector = message ? `.dc-ShortCut-alert[data-message="${message}"]` : '.dc-ShortCut-alert';
