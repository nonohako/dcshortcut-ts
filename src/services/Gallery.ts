  ...
  getInfo(): CurrentGalleryInfo {
    ...
    if (nameEl) {
      ...
      if (nameFromTextNodes) {
        ...
        console.warn(
          `getInfo: '${galleryId}' 갤러리의 이름을 찾지 못했습니다. ID를 이름으로 사용합니다.`
        );
      }
    } else {
      console.error(
        `getInfo: 갤러리 이름 선택자('div.fl.clear h2 a')를 찾지 못했습니다. ID를 이름으로 사용합니다.`
      );
    }
    ...
    try {
      ...
      if (keyExists) {
        ...
        if (galleryData && 'galleryId' in galleryData && 'galleryType' in galleryData) {
          ...
          console.error(
            `[handleFavoriteKey] 키 '${key}'는 존재하지만 유효한 갤러리 데이터를 가져오지 못했습니다.`
          );
